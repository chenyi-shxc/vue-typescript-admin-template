  ...
  <div class="app-container home">
    <el-row :gutter="20">
      <el-col :span="12" class="card-box">
        <el-card>
          <div slot="header"><span style="font-size: 22px">审计履职调查项目按机构分布</span></div>
          <div ref="charts" style="height: 700px">
          </div>
        </el-card>
      </el-col>
      ...
      <el-col :span="3" class="card-box">
        <el-card>
          <div><span>客户数 共{{dataTotal.custNum || 0}}个</span></div>
          <div><span>新增客户(月){{dataTotal.newCustNum || 0}}个</span></div>
        </el-card>
      </el-col>
      <el-col :span="3" class="card-box">
        <el-card>
          <div><span>项目数 共{{dataTotal.projectNum || 0}}个</span></div>
          <div><span>新增项目(月){{dataTotal.newProjectNum || 0}}个</span></div>
        </el-card>
      </el-col>
      <el-col :span="3" class="card-box">
        <el-card>
          <div><span>问题数 共{{dataTotal.surveyProblemNum || 0}}个</span></div>
          <div><span>新增问题数(月){{dataTotal.newSurveyProblemNum || 0}}个</span></div>
        </el-card>
      </el-col>
      <el-col :span="3" class="card-box">
        <el-card>
          <div><span>不良/核销金额</span></div>
          <div><span>{{dataTotal.surveyOutstanding || 0}}(亿元)</span></div>
        </el-card>
      </el-col>
      ...
      <el-col :span="6" class="card-box" style="height: 300px">
        <el-card>
          <div slot="header"><span style="font-size: 15px">待办事项</span></div>
          <div class="el-table el-table--enable-row-hover el-table--medium">
          <div>[通知] 系统升级</div>
            <div>[数据时点] 数据时点更新规则</div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6" class="card-box" style="height: 300px">
        <el-card>
          <div slot="header"><span style="font-size: 15px">我的收藏</span></div>
          <div class="el-table el-table--enable-row-hover el-table--medium">
            <div>XXXXXXX某某客户</div>
            <div>XXXXXXX某某项目 </div>
          </div>
      ...
      <el-col :span="12" class="card-box">
        <el-card>
        <div slot="header"><span style="font-size: 18px">审计履职调查项目表</span></div>
        <el-table
          :data="tableData"
          border
          style="width: 100%"
          ...
          <el-table-column
            prop="surveyProblemNum"
            label="问题数存量"
            width="120">
          </el-table-column>
          <el-table-column
            prop="newSurveyProblemNum"
            label="问题数新增"
            width="120">
          </el-table-column>
          <el-table-column
            prop="surveyOutstanding"
            label="不良/核销金额(亿元)"
            width="150">
          </el-table-column>
        </el-table>
...
import { listZkrBlocInfo233,listZkrBlocInfo244,listZkrBlocInfo255} from "@/api/survey/zkrBlocInfo";
...
export default {
  ...
  methods: {
    getdata(){
      listZkrBlocInfo233(this.queryParams).then((response)=>{
        this.dataTotal=response.dataList[0];
      });
    },
    getDataList(){
      listZkrBlocInfo255(this.queryParams).then((response)=>{
        console.log(response)
        response.data.forEach((item,index)=>{
          this.tableData.push({
            branchOne: item.branchOne,
            custNum: item.custNum,
            newCustNum: item.newCustNum,
            projectNum: item.projectNum,
            newProjectNum:item.newProjectNum,
            surveyProblemNum: item.surveyProblemNum,
            newSurveyProblemNum:item.newSurveyProblemNum,
            surveyOutstanding: item.surveyOutstanding,
          })
        });
      });
    },
    ...
    getmap() {
      listZkrBlocInfo244(this.queryParams).then((response)=>{
