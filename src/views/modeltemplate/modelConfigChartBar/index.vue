<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="图表标题名称" prop="columnTitle" label-width="100px">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['modeltemplate:modelConfigChartBar:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['modeltemplate:modelConfigChartBar:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['modeltemplate:modelConfigChartBar:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['modeltemplate:modelConfigChartBar:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="柱状图legend名称" prop="barLegendName">
          <el-input
            v-model="form.barLegendName"
            type="textarea"
            placeholder="请输入内容"
          />
        </el-form-item>
        <el-form-item label="柱状图列名称(用,分隔)" prop="barName">
          <el-input
        ...
        <el-form-item label="柱状图值名称(用,分隔)" prop="barValue">
          <el-input
            v-model="form.barValue"
            type="textarea"
            placeholder="请输入内容"
          />
        </el-form-item>
...
<script>
import {
  listModelConfigChartBar,
  getModelConfigChartBar,
  delModelConfigChartBar,
  addModelConfigChartBar,
  updateModelConfigChartBar,
  exportModelConfigChartBar
} from "@/api/modeltemplate/modelConfigChartBar";
import { listStModer } from "@/api/survey/stModer";

export default {
  name: "ModelConfigChartBar",
  components: {},
  data() {
    return {
      // 遮罩层
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listModelConfigChartBar(this.queryParams).then(response => {
        this.modelConfigChartBarList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getModelConfigChartBar(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改模板配置-列配置-柱状图";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateModelConfigChartBar(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addModelConfigChartBar(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm(
        '是否确认删除模板配置-列配置-柱状图编号为"' + ids + '"的数据项?',
        "警告",
        {
        ...
        .then(function() {
          return delModelConfigChartBar(ids);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有模板配置-列配置-柱状图数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(function() {
          return exportModelConfigChartBar(queryParams);
        })
        .then(response => {
